...
export function useInventory() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:4400/api/inventory/products');
  ...
  const fetchDashboardData = useCallback(async (productId, days) => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://localhost:4400/api/inventory/products/${productId}/dashboard?days=${days}`);
  ...
  useEffect(() => {
    if (selectedProductId) {
      fetchDashboardData(selectedProductId, period);
    }
